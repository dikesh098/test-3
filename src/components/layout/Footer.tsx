...
const SAAS_LINKS = [
  { label: "CRM SaaS Platform", href: "/services/crm-saas-platform" },
  { label: "Tech Support & Managed IT", href: "/services/tech-support-maintenance" },
...
export default function Footer() {
  return (
    <footer className="relative border-t border-line bg-surface-2 px-5 pb-8 pt-16 text-ink-2 sm:px-8">
      <div className="pointer-events-none absolute inset-x-0 top-0 h-px edge-gradient opacity-60" />
      <div className="mx-auto grid max-w-6xl gap-10 border-b border-line pb-12 sm:grid-cols-2 lg:grid-cols-4">
        <div>
        ...
        <FooterColumn title="Platform & Support" links={SAAS_LINKS} />
...
function FooterColumn({ title, links }: { title: string; links: { label: string; href: string }[] }) {
  return (
    <div>
      <h4 className="font-display mb-4 text-xs font-bold uppercase tracking-widest text-ink">{title}</h4>
      <ul className="flex flex-col gap-2.5">
        {links.map((l) => (
          <li key={l.label}>
            <Link href={l.href} className="text-sm transition hover:text-ink">
              {l.label}
            </Link>
          </li>
        ))}

...
export default function ServiceApplicationForm({
  ...
  return (
    <div className="rounded-2xl border border-line bg-surface-2 p-7 shadow-[var(--shadow-card)]" id="apply">
      <h3 className="font-display text-xl font-bold text-ink">Apply for {serviceName}</h3>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          ...
          <div
            ...
            className="flex cursor-pointer items-center gap-2 rounded-xl border-2 border-dashed border-line-2 bg-surface-2 px-4 py-3 text-sm transition hover:border-brand"
          >
            <Paperclip size={16} className="text-brand" />
            {doc ? (
              <span className="flex flex-1 items-center justify-between gap-2 font-medium text-ink">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          {payAmount > 0 && (
            <button
              ...
              className="glass glass-hover flex flex-1 items-center justify-center gap-2 rounded-xl border-2 border-brand py-3.5 text-sm font-semibold text-brand transition hover:-translate-y-0.5"
            >

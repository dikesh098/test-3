        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {PORTFOLIO.map((p, i) => (
            <Reveal key={p.name} delay={0.07 * i}>
              <div className="glass glass-hover relative overflow-hidden rounded-2xl p-6 shadow-[var(--shadow-card)] transition hover:-translate-y-1 hover:shadow-[var(--shadow-card-lg)]">
                ...
                <h3 className="font-display mt-4 text-base font-bold text-ink">{p.name}</h3>
                ...
                <div className="mt-5 grid grid-cols-3 gap-2 border-t border-line pt-4">
                  {p.stats.map((s) => (
                    <div key={s.label} className="text-center">
                      <div className={`font-display text-sm font-bold ${s.growing ? "text-brand" : "text-ink"}`}>
                        {s.value}
                      </div>

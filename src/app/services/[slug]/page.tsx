...
export default async function ServiceDetailPage({
  ...
  return (
    <>
      <PageHero eyebrow={service.category === "ai" ? "AI Agents" : service.category === "digital" ? "Digital Services" : "CRM & Support"} title={service.name} description={service.shortDesc} />
        ...
        <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[1.3fr_1fr]">
          <div className="space-y-12">
            ...
            <Reveal>
              <div>
                <h2 className="font-display flex items-center gap-2 text-xl font-bold text-ink">
                  <FileText size={20} className="text-brand" /> What We&apos;ll Need From You
                </h2>
                <ul className="mt-4 grid gap-2.5 sm:grid-cols-2">
                  {service.documents.map((d) => (
                    <li
                      key={d}
                      className="flex items-start gap-2.5 rounded-lg bg-surface-2/60 px-4 py-3 text-sm text-ink-2"
                    >
            ...
            <Reveal>
              <div>
                <h2 className="font-display text-xl font-bold text-ink">Process</h2>
              ...
              <Reveal>
                <div>
                  ...
                  <div className="mt-4 space-y-3">
                    {service.faqs.map((f) => (
                      <div key={f.question} className="glass rounded-xl p-5">
          ...
          <div className="lg:sticky lg:top-24 lg:self-start">
            <div className="glass mb-5 rounded-2xl p-6 shadow-[var(--shadow-card)]">
              <div className="text-xs font-bold uppercase tracking-widest text-ink-3">Pricing</div>
              <div className="font-display mt-1 text-2xl font-extrabold text-gradient">{service.priceDisplay}</div>

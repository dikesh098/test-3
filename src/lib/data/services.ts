...
export const SERVICES: Service[] = [
  // ---------------------------------------------------------------
  // DIGITAL — shown first across the whole site
  // ---------------------------------------------------------------
  {
...
export function getFeaturedServices(): Service[] {
  const digital = SERVICES.filter(
    (s) => s.featuredOnHome && (s.category === "digital" || s.category === "ai")
  );
  const govt = SERVICES.filter(
    (s) => s.featuredOnHome && s.category === "government"
  );
  return [...digital, ...govt];
}
